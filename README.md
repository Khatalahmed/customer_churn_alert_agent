...
<sub>AUC is reproducible and checked in CI. *Agent metrics are from an earlier LLM run, on a
dataset generated before the simulator clock was frozen — not yet re-run. See
[Agent metrics](#agent-metrics-earlier-run).</sub>
...
| **2 · Investigate** | LLM deep agent + 3 sub-agents | ~₹5/scan (earlier run) | **top 15** by risk × value |
...
One command → an evidence-backed retention worklist (real output from an earlier run; customer
ids and names differ on the current frozen dataset):
...
### Agent metrics (earlier run)

Precision, evidence fidelity and cost need a live LLM run, so they are **not** regenerated by
CI. The figures above come from an earlier run, before several changes: the dataset is now
generated from a frozen clock, tools pre-compute the evidence counts, and prices are read
from `.env`. They have not been re-measured since. To refresh them:
