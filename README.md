...
To host the pair — the API on Azure Container Apps, the interface on Vercel — follow
[`docs/DEPLOY.md`](docs/DEPLOY.md). GitHub Actions publishes the image to ghcr.io once CI
passes, and the image rebuilds the database and model from the seeded simulator, so nothing
large is ever uploaded.
