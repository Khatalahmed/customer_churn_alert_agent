#!/usr/bin/env python3
"""
Quick-Commerce Platform Simulator (Blinkit / Zepto style) -- CAUSAL CHURN
=========================================================================
Creates a local SQLite database, seeds reference + user data, backfills
>= 100 days of history, and can simulate live data generation.
Standard library only -- no external dependencies.

CHURN (causal version)
----------------------
Each customer has hidden "unhappiness drivers" (delivery_pain, support_pain,
pickiness). These drivers do two things at once:
  1. shape the customer's DATA  -> more cancels, more open tickets, low ratings
  2. raise the customer's CHANCE of churn
So churn is CAUSED by bad experience, not random. This makes churn
predictable from behaviour (real signal for an ML model, no leakage).
...
def seed_customers(conn: sqlite3.Connection, n: int, history_days: int, now: datetime) -> list[dict]:
    """
    Create n customers. Each gets hidden "unhappiness drivers" that CAUSE
    both the bad events in their data (cancels, tickets, low ratings) AND
    their chance of churn. This makes churn predictable from behaviour,
    not random.
    """
    cur = conn.cursor()
    customers = []
    ...
    for i in range(n):
        ...
        # --- NEW: hidden unhappiness drivers (0.0 = fine, 1.0 = very bad) ---
        # These shape the customer's data below AND drive their churn chance.
        delivery_pain = random.random()   # how often deliveries go wrong
        support_pain  = random.random()   # how often tickets stay unresolved
        pickiness     = random.random()   # how likely to give low ratings
        # overall unhappiness = weighted mix of the three drivers
        dissatisfaction = 0.40 * delivery_pain + 0.35 * support_pain + 0.25 * pickiness
...
def gen_order(conn, user, product_ids, day):
    ...
    # CHANGED: customers with high delivery_pain get more CANCELLED orders
    cancel_p = 0.05 + 0.25 * user.get("delivery_pain", 0.0)
    status = random.choices(
        ["DELIVERED", "CANCELLED", "OUT_FOR_DELIVERY"],
        weights=[max(0.05, 0.90 - cancel_p), cancel_p, 0.05], k=1,
    )[0]
...
def gen_ticket(conn, user, order_id, agent_ids, day):
    cur = conn.cursor()
    # CHANGED: high support_pain -> worse categories and more OPEN/unresolved
    if user.get("support_pain", 0.0) > 0.5:
        cat = random.choice(["DELIVERY_DELAY", "PAYMENT", "REFUND", "PRODUCT_QUALITY"])
        status = random.choices(TICKET_STATUS, weights=[0.40, 0.20, 0.15, 0.15, 0.10])[0]
    else:
        cat = random.choice(TICKET_CATEGORIES)
        status = random.choices(TICKET_STATUS, weights=[0.15, 0.10, 0.05, 0.40, 0.30])[0]
...
def gen_review(conn, user, product_id, order_id, day):
    cur = conn.cursor()
    # CHANGED: picky/unhappy customers give lower ratings
    if user.get("pickiness", 0.0) > 0.5:
        rating = random.choices([1, 2, 3, 4, 5], weights=[0.25, 0.25, 0.20, 0.20, 0.10])[0]
    else:
        rating = random.choices([1, 2, 3, 4, 5], weights=[0.03, 0.05, 0.12, 0.30, 0.50])[0]
...
def write_ground_truth(customers, path="churn_truth.json"):
    ...
    WHY : the database does not store the churn flag on purpose, because in
          real life you never know who will churn. But for testing our agent
          and model, we need the true answer, so we save it here in a
          separate file, not in the database. We also save the hidden drivers
          so we can study what causes churn.
    """
    truth = [
        {
            "user_id": c["user_id"],
